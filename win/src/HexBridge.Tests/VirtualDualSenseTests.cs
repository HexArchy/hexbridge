...
/// </summary>
public class VirtualDualSenseTests
{
    [Fact]
    ...
    {
        Assert.Equal("1-1", VirtualDualSense.BusIdFor(0));
        Assert.Equal("1-2", VirtualDualSense.BusIdFor(1));
    ...
    public void StringDescriptorsAnswerTheLanguageListAndTheNames()
    {
        ...
        Assert.Equal(device.ProductName, VirtualDualSense.DecodeString(product.Data));
    }
    ...
    public void AStringDescriptorSentByTheMacWinsOverTheFallback()
    {
        ...
        blocks.Add(new DescriptorBlock(DescriptorKind.StringDescriptor,
            [0x02, .. VirtualDualSense.EncodeString("DualSense Wireless Controller")]));

        using var device = new VirtualDualSense(new DeviceAttach(0, blocks), _ => { });
    ...
    {
        using var device = new VirtualDualSense(TestDevices.Attach(withFeatures: false), _ => { });
    ...
    public void AnAttachMissingTheDeviceDescriptorIsRefused()
    {
        ...
        Assert.Throws<FormatException>(() => new VirtualDualSense(new DeviceAttach(0, blocks), _ => { }));
    }
    ...
    public void BatteryComesOutOfByteFiftyThree(byte raw, string expected)
    {
        ...
        Assert.Equal(expected, VirtualDualSense.DescribeBattery(report));
    }

    [Fact]
    public void ATooShortReportHasNoBattery() =>
        Assert.Null(VirtualDualSense.DescribeBattery(new byte[10]));
}
