...
public sealed partial class UsbIpAttacher(string? explicitPath, Action<LogLevel, string> log)
{
    ...
    /// <summary>The vhci port the controller is plugged into, or null when it is not attached.</summary>
    ...
    public async Task<bool> AttachAsync(string busId, string host, CancellationToken token)
    {
        ...
        {
            log(LogLevel.Warning, $"usbip: attach не удался ({result.ExitCode}): {Summarise(result)}");
            return false;
        }

        _port = ParsePort(result.Output) ?? ParsePort(result.Error) ?? -1;
        log(LogLevel.Info, _port >= 0
            ? $"usbip: контроллер подключён к порту {_port}"
            : "usbip: контроллер подключён");
        return true;
    }

    /// <summary>
    /// Unplugs the device. Only ever by port: <c>detach --all</c> would also unplug whatever
    /// else the user has forwarded, which is not ours to do.
    /// </summary>
    public async Task DetachAsync(CancellationToken token)
    {
        ...
        if (result.ExitCode != 0)
        {
            log(LogLevel.Warning, $"usbip: detach порта {port} не удался: {Summarise(result)}");
        }
    }

    /// <summary>Forgets the attachment without running anything — used when the driver dropped it.</summary>
    public void Forget() => _port = -1;
