...
{
    private readonly Func<IUsbIpDevice?> _device;
    ...
    public UsbIpServer(Func<IUsbIpDevice?> device, Action<LogLevel, string> log)
    {
        _device = device;
        _log = log;
    }
    ...
    /// <summary>The device is imported and URBs are flowing.</summary>
    public bool IsImported => Volatile.Read(ref _imported) > 0;
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            switch (header.Code)
            {
                ...
                case UsbIpProtocol.OpReqImport:
                {
                    ...
                    var device = _device();
                    if (device is null || device.Info.BusId != busId || IsImported)
                    {
                        _log(LogLevel.Warning, $"usbip: запрошен busid {busId}, отдать нечего");
                        await Send(stream,
                    ...
                    {
                        Interlocked.Decrement(ref _imported);
    ...
    private byte[] DeviceList()
    {
        ...
        var reply = new byte[UsbIpProtocol.OpHeaderSize + 4 + entry.Length];
        OpHeader.Reply(UsbIpProtocol.OpRepDevList).Write(reply);
        System.Buffers.Binary.BinaryPrimitives.WriteUInt32BigEndian(
            reply.AsSpan(UsbIpProtocol.OpHeaderSize), device is null ? 0u : 1u);
        entry.CopyTo(reply, UsbIpProtocol.OpHeaderSize + 4);
        return reply;
    }
