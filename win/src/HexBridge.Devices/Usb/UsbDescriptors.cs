...
/// <summary>
/// The configuration descriptor as we present it to Windows: the first HID interface and
/// nothing else.
///
/// Plenty of forwardable hardware is composite. A DualSense puts audio interfaces for the
/// headset jack and the HD haptics ahead of its HID interface; wheels bring force-feedback
/// and hub interfaces along. Passing those through would mean serving endpoints the device
/// channel does not carry — isochronous audio above all — and Windows would sit waiting on
/// a pin that never speaks. So the configuration is rebuilt around the one interface we can
/// honour, renumbered to 0: a configuration that claims one interface and calls it number 3
/// is one Windows is entitled to distrust.
///
/// This is also why the Mac opens the lowest-numbered HID interface of a device: both ends
/// have to mean the same interface by "the HID one".
/// </summary>
