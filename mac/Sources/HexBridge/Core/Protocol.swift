...
enum Wire {
    ...
    enum PacketType: UInt8 {
        ...
        case haptic = 13
        /// Где вторая сторона, как её видит релей. Релей → обе стороны.
        ///
        /// Единственный тип, полезная нагрузка которого **не** зашифрована, и
        /// иначе быть не может: релей единственный видит оба публичных адреса
        /// и единственный, у кого нет ключа. На веру тут ничего не берётся —
        /// это адрес, который стоит попробовать, а путь считается рабочим
        /// только когда оттуда придёт расшифровавшийся пакет.
        case peer = 14
    }
